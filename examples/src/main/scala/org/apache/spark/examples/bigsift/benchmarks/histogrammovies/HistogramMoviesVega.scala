...
import org.apache.spark.examples.bigsift.bigsift.{DDNonExhaustiveVega, SequentialSplit, DDNonExhaustive}
...
object HistogramMoviesVega {
  ...
  def main(args: Array[String]) {
    try {
      ...
      val delta_debug = new DDNonExhaustiveVega[String, (Double, Int)]
      delta_debug.setMoveToLocalThreshold(local);
      val test = new TestVega
      test.setInverse(inverse)
      val returnedRDD = delta_debug.ddgen(showMeRdd, test, new SequentialSplit[String], lm, fh, DeltaDebuggingStartTime)
  ...
  def inverse(rdd1: Array[(Double,Int)]  , rdd2:Array[(Double,Int)]): Array[(Double, Int)] ={
    val map1 = scala.collection.mutable.HashMap[Double, Int]()
    val map2 = scala.collection.mutable.HashMap[Double, Int]()
