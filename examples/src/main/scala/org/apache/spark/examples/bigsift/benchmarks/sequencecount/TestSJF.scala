...
class TestSJF(fault : (String,String) => Tuple2[String, Int]) extends Testing[String] with Serializable {
	var num = 0;
	...
	def usrTest(inputRDD: RDD[String], lm: LogManager, fh: FileHandler ): Boolean = {
		...
		val finalRdd = inputRDD.filter(s => TriSequenceCount.filterSym(s)).flatMap(s => {
			...
			while (itr.hasMoreTokens) {
				...
				if (wordStringP1.equals("")) {
					...
					sequenceList += fault(s, finalString)
		...
		val out = finalRdd.filter(s => TriSequenceCount.failure(s)).collect()
		num = num + 1
	...
	def usrTest(inputRDD: Array[String], lm: LogManager, fh: FileHandler): Boolean = {
		...
		val out = finalRdd.filter(s => TriSequenceCount.failure(s))
		num = num + 1
		println( s""" >>>>>>>>>>>>>>>>>>>>>>>>>> The number of runs are $num <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,""")
		for (o <- out) {
			//  println(o)
