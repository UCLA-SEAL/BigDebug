...
object TriSequenceCountOverlap {
  def main(args: Array[String]): Unit = {
      ...
      val sequence = lines.filter(s => TriSequenceCount.filterSym(s)).flatMap(s => {
        ...
        while (itr.hasMoreTokens) {
          ...
          if (wordStringP1.equals("")) {
            ...
            sequenceList += TriSequenceCountOverlap.addFault(s, finalString)
          }
        }
        sequenceList.toList
    ...
    def runDD(maprdd : RDD[String] , logger: Logger , local:Int ,  lm: LogManager, fh: FileHandler): Boolean ={
      ...
      val foundfault = delta_debug.ddgen(maprdd, new TestSJF(addFault), new SequentialSplit[String], lm, fh, DeltaDebuggingStartTime)
      /** ************************
        * Time Logging
        * *************************/
...
//      return Tuple2(key, -9999999)
    if (str.contains("most tenacious") && key.contains("one|of|the")) // http://scai05.cs.ucla.edu:50075/browseBlock.jsp?blockId=-3751596619526339756&blockSize=67108864&filename=%2Fclash%2Fdatasets%2FWW%2Fwiki-size-80000000000&datanodePort=50010&genstamp=5739306&namenodeInfoPort=50070&chunkSizeToView=32768
      return Tuple2(key, -9999999)
    else if (str.contains("most tenacious") && key.contains("of|the|most")) // http://scai05.cs.ucla.edu:50075/browseBlock.jsp?blockId=-3751596619526339756&blockSize=67108864&filename=%2Fclash%2Fdatasets%2FWW%2Fwiki-size-80000000000&datanodePort=50010&genstamp=5739306&namenodeInfoPort=50070&chunkSizeToView=32768
      return Tuple2(key, -9999999)
