...
object SparkWordCountIntersect {
...
//    linRdd.show
    var linRdd = counts.getLineage()
    linRdd.collect()//.foreach(println)
    var linRdd1 = linRdd.filter(_ == 6232546377L).goBackAll()
    linRdd1.collect().foreach(println)
    linRdd = counts.getLineage()
    var linRdd2 = linRdd.goBackAll()
    linRdd2.collect()
    linRdd1.intersection(linRdd2).foreach(println)
