...
object L3 {
  def main(args: Array[String]) {
    ...
    val pageViews = lc.textFile(pageViewsPath, 1600)
    val users = lc.textFile(usersPath, 1600)
    ...
    val E = C.map(x => (x._1, x._2._1)).reduceByKey(_ + _)//groupBy(_._1)

    //val E = D.map(x => (x._1, x._2.reduce((a, b) => (a._1 + b._1, a._2 + b._2, a._3 + b._3)))).map(x => (x._1, x._2._3))

    if(saveToHdfs) {
      E.saveAsTextFile("hdfs://scai01.cs.ucla.edu:9000/clash/lineage/output-L3-" + args(1) + "G")
