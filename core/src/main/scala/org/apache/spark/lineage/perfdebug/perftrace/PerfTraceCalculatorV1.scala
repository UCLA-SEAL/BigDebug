...
import org.apache.spark.lineage.perfdebug.utils.{PRIdTuplePartitioner, PerfLineageUtils}
import org.apache.spark.lineage.rdd.{TapHadoopLRDD, _}
import org.apache.spark.rdd.RDD
import org.apache.spark.rdd.RDD._

/** First implementation of [[PerfTraceCalculator]]. This class presents a cleaner/more
 * straightforward abstraction in that intermediate stages always form a [[PerfLineageCache]],
 * which is similar to an RDD[(ID, (CacheValue, Long))]. While simpler, this may also incur extra
 * memory overhead since most of the [[CacheValue]] instances can be discarded earlier in the
 * computation flow.
 *
 * One alternative implementation can be found in [[PerfTraceCalculatorV2]], which discards the
 * [[CacheValue]] instances as early as possible. However, it also tries to preserve some degree
 * of code readability and is not fully optimized for the final join. See the documentation for
 * details.
 *
 * An improvement on both approaches would be a hybrid of the two: Discarding [[CacheValue]]
 * instances for all intermediate stages, but retaining it for the final output stage. However,
 * this does increase code complexity a bit (as there would be near-duplicate code blocks). This
 * may become an additional implementation in the future.
 *
 * Finally, a fourth possibility of this implementation is to compute latencies in a signle pass,
 * under the assumption that accFn is commutative (eg summing). Further brainstorming is required
 * for this approach, but the high-level goal would be for the intermediate stages to consist of
 * (InputId, (FinalOutputId*, Latency*)). Once all stages have been traced, this would then be
...
case class PerfTraceCalculatorV1(@transient initWrapper: LineageWrapper,
  ...
  override def calculate(): DefaultPerfLineageWrapper = {
   ...
   *  stats. */
  def joinInputRddsWithAgg(aggDep: LineageCacheDependencies,
