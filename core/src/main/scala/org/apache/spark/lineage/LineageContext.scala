...
class LineageContext(@transient val sparkContext: SparkContext) extends Logging {
	 ...
	 */
	private def createListenerAndUI(sc: SparkContext): BDSparkListener = {
		val listener = new BDSparkListener()
	...
	def runJobWithId[T, U: ClassTag](rdd: Lineage[T], func: Iterator[(T, Long)] => U): Array[U] = {
		...
		runJobFromSC(tappedRdd, (c: TaskContext, it: Iterator[(T, Long)]) => func(it), 0 until tappedRdd.partitions.size)
	}
	...
	def runJob[T: ClassTag, U: ClassTag](rdd: Lineage[T], func: Iterator[T] => U): Array[U] = {
		...
		//    sparkContext.runJob(tappedRdd, func, 0 until tappedRdd.partitions.size, false)
		runJobFromSC(tappedRdd, (c: TaskContext, it: Iterator[T]) => func(it), 0 until tappedRdd.partitions.size)
	...
	def runJob[T, U: ClassTag](rdd: Lineage[T], func: (TaskContext, Iterator[T]) => U): Array[U] = {
		...
		runJobFromSC(tappedRdd, func, 0 until rdd.partitions.size)
	}


	def runJobFromSC[T, U: ClassTag](
		                                rdd: RDD[T],
		                                func: (TaskContext, Iterator[T]) => U,
		                                partitions: Seq[Int])
	: Array[U] = {
		/** Initiating and setting variable in debugging manager --Tag @ Gulzar 06/23 */
		var jobid = 0;
