...
trait Lineage[T] extends RDD[T] {
	...
	def filterWithProfiling(f: T => Boolean, enableProfiling: Boolean = true): Lineage[T] = withScope {
		...
		val d_url = BDDriverBackend.driverDebuggerUrl
		val sparkappconfig  = BDDriverBackend.getSparkAppConfig()
		val temp_conf = BDDriverBackend.conf
		new MapPartitionsLRDD[T, T](
			this,
			(context, pid, iter) => {
				// Introducing initialization of comm channel for the debugger -- Tag Bigdebug @ Gulzar 07/12
				BDExecutorBackend.getBDExecutorBackend(temp_conf,sparkappconfig,d_url)
	...
	/**
	 * Return a new watchpoint lineage object.
	 */
	def watchpoint(f: T => Boolean): Lineage[T] = withScope {
		val cleanF = context.clean(f)
		val watch_rdd = new WatchPointLRDD[T, T](
			this,
			(context, pid, iter) => {
				// instrumenting iterators for debugging purposes -- Tag: Bigdebug @ Gulzar 6/16
				if (context.bdconfig.ENABLE_CRASH_LATENCY) {
					val watchpointIterator = new WatchpointInterruptableIterator[T](context, iter, id + 1)
					//TODO: Check if the id is needed to be incremented or not.
					watchpointIterator.filter(cleanF)
				} else
					iter.filter(cleanF)
			},
			preservesPartitioning = true)
...
object Lineage {
	...
	def findRDDTypeAndFix(rdd: RDD[_], code: String): String = {
		rdd match {
			case _: MapPartitionsRDD[_, _] =>
				return rdd.asInstanceOf[MapPartitionsLRDD[_, _]].reCompileFilter(code)
			case _ =>
				println("patch not supported")
		}
		""

	}

	def findCrashedRDDTypeAndFix(rdd: RDD[_], code: String, crashedRecords: Iterator[CrashingRecord]): Iterator[CrashingRecord] = {
		rdd match {
			case _: MapPartitionsLRDD[_, _] =>
				return rdd.asInstanceOf[MapPartitionsLRDD[_, _]].batchRemediation(code, crashedRecords)
			case _ =>
				println("patch not supported")
				return List[CrashingRecord]().toIterator
		}
	}
