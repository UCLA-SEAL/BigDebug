 ...
 */
@DeveloperApi
class IntKeyAppendOnlyMap[V](initialCapacity: Int = 64)
  ...
  private val haveNullValue = false
  ...
  def update(key: Int, value: V): Unit = {
    ...
    while (true) {
      val curKey = data(2 * pos)
      if (curKey.eq(null)) {
        data(2 * pos) = key.asInstanceOf[AnyRef]
  ...
  protected def growTable() {
    ...
    while (oldPos < capacity) {
      if (!data(2 * oldPos).eq(null)) {
        ...
        var newPos = key.asInstanceOf[Int] & newMask
        var i = 1
        var keepGoing = true
        while (keepGoing) {
