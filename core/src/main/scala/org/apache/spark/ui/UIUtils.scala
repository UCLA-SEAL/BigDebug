...
private[spark] object UIUtils extends Logging {
	...
	def commonHeaderNodes: Seq[Node] = {
			<meta http-equiv="Content-type" content="text/html; charset=utf-8"/>
				...
				<link rel="stylesheet" href={prependBaseUri("/static/codemirror.css")} type="text/css"/>
				<link rel="stylesheet" href={prependBaseUri("/static/theme/ambiance.css")} type="text/css"/>
			...
			<script src={prependBaseUri("/static/timeline-view.js")}></script>
				<script src={prependBaseUri("/static/codemirror.js")}></script>
				<script src={prependBaseUri("/static/mode/clike/clike.js")}></script>
				<script src={prependBaseUri("/static/addon/edit/matchbrackets.js")}></script>
				<script src={prependBaseUri("/static/addon/selection/active-line.js")}></script>
	...
	def headerSparkPage(
		                   ...
		                   onload: String = null // Initiating UI for debugging features -- Tag Bigdebug @Gulzar 06/22
		                   ): Seq[Node] = {
		...
		<html>
			<head>
				{commonHeaderNodes}{if (showVisualization) vizHeaderNodes else Seq.empty}{if (useDataTables) dataTablesHeaderNodes else Seq.empty}<title>
				{appName}
				-
				{title}
			</title>
	...
	def makeProgressBar(
		...
		<div class="progress">
			<span style="text-align:center; position:absolute; width:100%; left:0;">
				{completed}
				/
				{total}{if (failed > 0) s"($failed failed)"}{if (skipped > 0) s"($skipped skipped)"}{if (killed > 0) s"($killed killed)"}
			</span>
	...
	def makeDescription(desc: String, basePathUri: String, plainText: Boolean = false): NodeSeq = {
		...
		try {
			...
			case NonFatal(e) =>
				if (plainText) Text(desc)
				else <span class="description-input">
					{desc}
				</span>
		}
	}
