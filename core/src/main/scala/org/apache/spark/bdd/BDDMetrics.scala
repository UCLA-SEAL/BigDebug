...
/**Fix it by adding it as a parent to this BDDMETric classs.**/
  ...
  var rddID: Int = -1
  var recordPendingStack = HashMap[(Int, Int), (Long, String)]()
  ...
  def rememberRecordEntryTime(kv: String, sID: Int, taskID: Int, rdd: Int) {
    recordPendingStack.synchronized {
    if (sID != rddID) {
      rddID = sID
      recordPendingStack = recordPendingStack.drop(recordPendingStack.size)
    }
    val time = System.currentTimeMillis()

      recordPendingStack((taskID, rdd)) = (time, kv)
    }
  }

  def rememberRecordEntryTime(kv: String, sID: Int, taskID: Int, rdd: Int, time: Long) {
    recordPendingStack.synchronized {
    if (sID != rddID) {
      rddID = sID
      recordPendingStack = recordPendingStack.drop(recordPendingStack.size)
  ...
  def recordDoneNotification(sID: Int, taskID: Int, rdd: Int) {
    val time = System.currentTimeMillis()
    var intermediate: Tuple2[Long, String] = null
    recordPendingStack.synchronized {
      if (recordPendingStack.size > 0) {
        val Some(temp) = recordPendingStack.remove((taskID, rdd))
        intermediate = temp
      } else
        return
    }
    recordAvgStd.synchronized {
      intermediate match {
        case (stime, record) =>
          val span = time - stime
          var (sumSQ, sum, n, prevRecord) = recordAvgStd.getOrElse((sID, taskID, rdd), (0L, 0L, 0, null))
          ...
          DebugHelper.log("INFO", "BDDMetric", s"** Current Record(rddID: $sID, taskID: $taskID, (Time Spent: $span, Record: $record **")
          if (prevRecord == null) {
            recordAvgStd((sID, taskID, rdd)) = (sumSQ, sum, n, record)
            insertInTop10(rdd, record, span)
          }
          else {
            if (span > ((bdconfig.KDEV * std) + avg) && span > 1) {
              DebugHelper.log("INFO", "BDDMetric", s"** Straggling Record(rddID: $sID, taskID: $taskID, (Time Spent: $span, Record: $record **")
              insertInTop10(rdd, record, span)
              recordAvgStd((sID, taskID, rdd)) = (sumSQ, sum, n, record)
              updateRecordProfileUI(rdd)
            } else {
              recordAvgStd((sID, taskID, rdd)) = (sumSQ, sum, n, prevRecord)
            }
          }
        case _ =>
          recordPendingStack = HashMap[(Int, Int), (Long, String)]()
      }
  ...
  def getUiProfileData(rdd: Int): String = {
    ...
    val avg = sum.asInstanceOf[Float] / n.asInstanceOf[Float]
    val std = Math.sqrt(((sumSQ.asInstanceOf[Float] / n.asInstanceOf[Float]) - (avg * avg)))
    str = "[" + {if(str.endsWith(",")) str else str + ","} + s"""{"label": "Average", "y":$avg},{"stddev":$std}]"""
