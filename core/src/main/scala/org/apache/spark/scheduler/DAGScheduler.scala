...
class DAGScheduler(
  ...
  def getWaitingStages() : HashSet[Stage] =
  {
    this.waitingStages
  }

  def getStage(stageId: Int): Stage = {
    var obj = this.waitingStages.find(e => e.id == stageId).getOrElse(null)
    if (obj == null) {
      obj = this.runningStages.find(e => e.id == stageId).getOrElse(null)
    }
    obj
  }
}
