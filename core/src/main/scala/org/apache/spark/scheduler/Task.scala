...
private[spark] abstract class Task[T](val stageId: Int, var partitionId: Int) extends Serializable {
  ...
  final def run(attemptId: Long): T = {
    println("## TaskContext is created, stageId:" + stageId + ", partitionId:" + partitionId + ", attemptId:" + attemptId )
    ...
    if(TaskExecutionManager.isFirst(stageId)) { // TODO: remove hardcording
      TaskExecutionManager.SetBreakpointBeforeRunning(stageId, partitionId, true)
      TaskExecutionManager.SetBreakpointAfterRunning(stageId, partitionId, true)
    }

    // Inquiry whether the task should be run or not
    // TODO: however, this place is not the appropriate code location for setting a breakpoint. This is just a hack.
    TaskExecutionManager.CheckBreakPointBeforeRunning(stageId, partitionId)
    val ret = runTask(context)
    // Inquiry whether the task should be completed or not.
    TaskExecutionManager.CheckBreakPointAfterRunning(stageId, partitionId)
